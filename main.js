...
const ambientLight = new THREE.AmbientLight(0xffffff, 5.15);  // Soft white ambient light
...
const spotLight3 = new THREE.SpotLight(0xff00e1, 1008500, 0, Math.PI / 6, 1);
...
const loader = new GLTFLoader().setPath('public/panorama/');
loader.load(
  'scene.gltf',
  (gltf) => {
  ...
  if (townhallMesh && scene.children.includes(townhallMesh)) {
    ...
    loader2.load(
      ...
      (gltf) => {
        ...
        townhallMesh.scale.set(4.4, 4.4, 4.4);
        townhallMesh.position.set(17, 11, -13);   // Set the position in the scene
